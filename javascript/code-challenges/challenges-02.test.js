...
const evenOdd = (arr) => {
  let evenoddArr = arr.map((num)=>{
    ...
    else if (Number.isInteger(num/n) === false){
      return 'odd';
    }
    else {return 'N/A'}
  });
...
const extractStats = (arr) => {
  let arrNew = arr.map(item => ({ name : item.stat.name, total : item.effort+item.baseStat,
  }));
  return arrNew;
};
