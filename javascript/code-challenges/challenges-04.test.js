...
const updateAnimal = (arr, callback) => {
  let array = arr.map((str)=> callback(str));
  return array;
};
...
const sortNumbersByLength = (arr) => {
  let convertToStrings= arr.map((num)=>JSON.stringify(num));
  let sortStrings = convertToStrings.sort((a,b) => a.length - b.length);
  let sortNum = sortStrings.map((num)=>parseFloat(num));
  return sortNum;
};
