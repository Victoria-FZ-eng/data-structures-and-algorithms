...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.map((ing)=>{return result.push(ing.split('').slice().join(''));
  })
  return result;
};
...
const stepActions = (recipe) => {
  let result = [];
  recipe.steps.map((ing)=>{return result.push(ing.split(" ")[0]);
  })
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (str.length < numberOfCharacters){
    ...
    modstr=str.substring(0, str.length - numberOfCharacters)
  }
  return modstr;
};
