...
const sortByChildren = (charArray) => {
  let sortedByChildrenNumbers= charArray.sort((obja, objb)=>{
    if (obja.children.length > objb.children.length){
      return 1;
    }else if(obja.children.length < objb.children.length){
      return -1;
    }else{
        if (obja.name.length > objb.name.length){
           return 1;
        }else {
           return -1;
        }
  ...
  return sortedByChildrenNumbers;
};
...
const citiesAtoJ = (arr) => {
  let regex1 = /[A-J][a-zA-Z]*/g;

  let array= arr.filter((str)=>{
     if(str.match(regex1) == str){
 return str;
     }
...
const matchMonth = (input) => {
  let regex1 = /[o][c][t]*/i;
  let checkVals= ['October', 'Oct', 'october', 'oct'].filter(entered =>{
    if (entered === input){
      return true;
    }});

    return checkVals[0] && input.match(regex1);
...
const noPunctuation = str => {
  let arr =str.split();
 // let newArr = arr.filter((strin)=>(/ \s$/.test(strin)));
  //let newArr = arr.filter((strin)=>(strin.endsWith(' ')));
  return arr;
};
